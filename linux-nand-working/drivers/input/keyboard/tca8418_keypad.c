...
//		printk("%s tca8418 write byte failed failed,reg: %d, val: %d, error: %d\n",
//				__func__, reg, val, error);

		dev_err(&keypad_data->client->dev,
				"%s tca8418 write byte failed failed, reg: %d, val: %d, error: %d\n",
				__func__, reg, val, error);
		return error;
	}
		...
		u8 *val)
{
	...
	if (error < 0) {
//		printk("%s tca8418 Read byte failed failed,reg: %d, val: %d, error: %d\n",
//				__func__, reg, val, error);

		dev_err(&keypad_data->client->dev,
				"%s tca8418 Read byte failed failed, reg: %d, error: %d\n",
				__func__, reg, error);
		return error;
	}
				   ...
				   const struct i2c_device_id *id)
{
	...
	keypad_data = kzalloc(sizeof(*keypad_data) + max_keys * sizeof(keypad_data->keymap[0]), GFP_KERNEL);
	if (!keypad_data)
		return -ENOMEM;
...
//	input->phys = "tca8418_keypad/input0";
...
static void __exit tca8418_keypad_exit(void)
{
 ...
 MODULE_AUTHOR("Francis Kumar <francis.kumar@stesalit-inc.com>");
