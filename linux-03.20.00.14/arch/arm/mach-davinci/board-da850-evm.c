...
//sree lcd backlight nand flash
#define DA850_LCD_BACKLIGHT		GPIO_TO_PIN(5, 11)
//sree lcd backlight nand flash
...
static struct davinci_nand_pdata da850_evm_nandflash_data = {
	...
	.ecc_bits	= 4,
	.ecclayout	= &dm365_evm_nand_ecclayout,  //sree-nand-patch-ecc-error
...
static struct platform_device *da850_evm_devices[] __initdata = {
	...
	//sree-nand&da850_evm_norflash_device,
};
...
//Buzzzz Modified on 10/09/2011****
	...
	if (!HAS_MMC) {
		...
		ret = da8xx_pinmux_setup(da850_nor_pins); nand-sree
		if (ret)
...
//sree-nand	da850_evm_setup_nor_nand();
...
static int da850_lcd_hw_init(void)
{
	...
	if (status < 0)
		return status;
	/*sree lcd backlight nand flash
	status = gpio_request(DA850_LCD_BACKLIGHT, "lcd bl sree \n");
	if (status)
	{
		pr_warning("Sreenivas can not open LCD BACK LIGHT CONTROL PIN %d\n",
				DA850_LCD_BACKLIGHT);
		return status;
	}
	status = gpio_direction_output(DA850_LCD_BACKLIGHT, 0);
	if (status)
	{
		pr_warning("Sreenivas can not set as output as LCD BACK LIGHT CONTROL PIN %d\n",
				DA850_LCD_BACKLIGHT);
		return status;
	}
 	gpio_set_value(DA850_LCD_BACKLIGHT, 1);
	//sree lcd backlight nand flash	*/
...
/*	sharp_lk043t1dg01_pdata.panel_power_ctrl = da850_panel_power_ctrl,
	...
	da850_evm_usb_init();
	da850_evm_setup_nor_nand();	//sree-nand
