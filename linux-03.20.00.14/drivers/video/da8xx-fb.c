...
static struct da8xx_panel known_lcd_panels[] = {
	...
	[2] = {
		...
		.name = "Innolux_AT050TN33",
		.width = 480,
		.height = 272,
		.hfp = 22,				/* The 8-bit horizontal front porch (HFP) field is used to specify the number of dummy pixel clocks to insert at
									the end of each line or row of pixels before pulsing the line clock(or LCD_HSYNC) pin.*/
		.hbp = 19,				/* The 8-bit horizontal back porch (HBP) field is used to specify the number of dummy pixel clocks to insert
									at the beginning of each line or row of pixels */
		.hsw = 4,				/* The 6-bit horizontal synchronization pulse width (HSW) field is used to specify the pulse width of the line
									clock in active mode */
		.vfp = 6,
		.vbp = 9,
		.vsw = 1,
		.pxl_clk = 9000000,
		.invert_pxl_clk = 0,	/* The invert pixel clock (IPC) bit is used to select the edge of the pixel clock that drives pixel data out onto
									the LCD data lines. */
...
MODULE_AUTHOR("Stesalit, Hyderabad");
MODULE_LICENSE("GPL");
